...
export class Vpc extends ComponentResource {
    ...
    public readonly publicSecurityGroupIds: Output<string>[];
    public readonly privateSecurityGroupIds: Output<string>[];
    ...
    constructor(name: string, args: VpcArgs, opts?: pulumi.ComponentResourceOptions) {
        ...
        [this.publicSubnets, this.publicSecurityGroupIds] = this.createPublicSubnets(_publicSubnetsSettings);
        [this.privateSubnets, this.privateSecurityGroupIds] = this.createPrivateSubnets(_privateSubnetsSettings);
     ...
     */
    private createPublicSubnets(subnetsSettings: subnetArgs[]): [Output<aws.ec2.Subnet[]>, Output<string>[]] {

        let subnetIds: Output<string>[] = [];
        ...
        subnetsSettings.forEach(subnetSettings => {
            ...
            subnetIds.push(vpcSubnet.id);

        });

        return [pulumi.output(subnets), subnetIds];
    }
     ...
     * @returns returns an array with 2 or more Vpc Subnets
     */
     private createPrivateSubnets(subnetsSettings: subnetArgs[]): [Output<aws.ec2.Subnet[]>, Output<string>[]] {
    ...
    private createSecurityGroups(): [Output<aws.ec2.SecurityGroup[]>, Output<string>[]] {
        ...
        _sg = new aws.ec2.SecurityGroup(sg_name, {
            vpcId: this.vpc.id,
            description: 'Allow SSH, HTTP and TLS inbound traffic',
            tags: {
                'Name': sg_name,
                'Requested-by': this.args.ownerEmail,
            },
